...
public class PixelClassification extends AbstractIlastikExecutor {
    ...
    public PixelClassification( File executableFilePath, File projectFileName, LogService logService, StatusService statusService, int numThreads, int maxRamMb )
    {
        super( executableFilePath, projectFileName, logService, statusService, numThreads, maxRamMb );
    }

    public <T extends NativeType<T>> ImgPlus<T> classifyPixels( ImgPlus<? extends RealType<?>> rawInputImg, PixelPredictionType pixelPredictionType ) throws IOException
    {
